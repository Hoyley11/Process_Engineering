...
def initialize_master_list():
    ...
    if not MASTER_LIST_PATH.exists():
        df = pd.DataFrame(columns=[
            "Tag", "Title", "Type", "Status", "Description", 
...
def save_equipment_sizing(equipment_tag, sizing_results):
    ...
    # Preserve existing title if it exists so we don't wipe it during a sizing save
    existing_title = ""
    if equipment_tag in df['Tag'].values:
        existing_row = df[df['Tag'] == equipment_tag]
        if 'Title' in existing_row.columns and pd.notna(existing_row['Title'].values[0]):
            existing_title = existing_row['Title'].values[0]
